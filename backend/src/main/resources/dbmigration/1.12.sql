...
alter table ingredients add column if not exists volumic_mass float default 1f not null;
alter table ingredients add column if not exists weight_per_unit float default 1f not null;
